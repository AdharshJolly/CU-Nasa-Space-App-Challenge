...
const CoordinatorCard = ({ name, role, avatar, hint, email, phone }: { name: string, role: string, avatar: string, hint: string, email: string, phone: string }) => (
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">
                Have a question or need assistance? Reach out to our coordination team.
            </p>
        </div>

        <div className="space-y-12">
            <div>
                <h3 className="text-2xl font-headline font-bold text-center mb-8 text-primary">Faculty Coordinators</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
                    {facultyCoordinators.map(person => <CoordinatorCard key={person.name} {...person} />)}
                </div>
            </div>

            <div>
                 <h3 className="text-2xl font-headline font-bold text-center mb-8 text-primary">Student Coordinators</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-4xl mx-auto">
                    {studentCoordinators.map(person => <CoordinatorCard key={person.name} {...person} />)}
                </div>
            </div>
        </div>
      </div>
