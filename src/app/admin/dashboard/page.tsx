...
export default function AdminDashboard() {
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          if (
            ...
            data.scheduledChange.toDate() < new Date()
          ) {
            ...
            logActivity(user?.email, "Scheduled Registration Change Executed", {
              newState: data.scheduledState,
            });
          } else {
  ...
  const handleRegistrationSettingsChange = async (
    newSettings: RegistrationSettings
  ) => {
    try {
      const settingsToSave: any = {
        ...newSettings,
        scheduledChange: newSettings.scheduledChange
          ? Timestamp.fromDate(newSettings.scheduledChange)
          : null,
      };
      await setDoc(doc(db, "settings", "registration"), settingsToSave);

      let toastDescription = `Registrations are now ${
        newSettings.enabled ? "OPEN" : "CLOSED"
      }.`;
  ...
  const handleSaveUser = async (data: {
    ...
    try {
      ...
      await logActivity(
        ...
        { targetUser: data.email, role: data.role }
      );
